...
function winner(deckSteve, deckJosh) {
    ...
    const totalOfCards = deckSteve.length;

    const cardRank = ['2', '3', '4', '5', '6', '7', '8', '9', 'T', 'J', 'Q', 'K', 'A']

    let cardsSteve = deckSteve.map(x => cardRank.indexOf(x));
    let cardsJosh = deckJosh.map(x => cardRank.indexOf(x));

    let scoreSteve = 0
    let scoreJosh = 0

    for (let i = 0; i < totalOfCards; i++) {
        if (cardsSteve[i] > cardsJosh[i]) {
            scoreSteve++
        }
        if (cardsSteve[i] < cardsJosh[i]) {
            scoreJosh++
    ...
    switch (true) {
        case scoreSteve > scoreJosh:
            return `Steve wins ${scoreSteve} to ${scoreJosh}`
        case scoreJosh > scoreSteve:
